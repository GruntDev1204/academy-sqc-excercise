...
public class EmployeesRequest implements RequestInterface<Employee> {
    ...
    public boolean checkRequest(Employee employeeRequestData) {
        ...
        if (employeeRequestData.getFullName() == null ||employeeRequestData.getFullName().isEmpty()) detailError.put("fullName", "Full name cannot be empty!");

        if (employeeRequestData.getBirthDay() == null) detailError.put("birthDay", "Birth date cannot be null or empty!");

        if (employeeRequestData.getGender() == null || employeeRequestData.getGender().isEmpty())  detailError.put("gender", "Gender cannot null or empty!");
        else if (!employeeRequestData.getGender().equals("Male") && !employeeRequestData.getGender().equals("Female")) detailError.put("gender", "Gender must be Male or Female");

        if (employeeRequestData.getSalary() <= 0)   detailError.put("salary", "Salary must be greater than zero");

        if (employeeRequestData.getPhoneNumber() == null || employeeRequestData.getPhoneNumber().isEmpty())  detailError.put("phoneNumber", "Phone number cannot be empty");

        if (employeeRequestData.getDepartmentId() == null)  detailError.put("departmentId", "Department ID cannot be null");
