...
public class DepartmentController implements RestControllerInterface<Department, Object> {
    ...
    public ResponseEntity<ApiResponse<Department>> create(@RequestBody Department data) {
        return res.returnResponseJson(SuccessCode.CREATE_SUCCESS, departmentService.create(data));
    }
    ...
    @Override
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Department>> update(@PathVariable Integer id, @RequestBody Department newData) {
        return res.returnResponseJson(SuccessCode.UPDATE_SUCCESS, departmentService.update(id, newData));
    }
